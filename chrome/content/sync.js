...
function init() {
  try {
    syncsvc = Cc["@mozilla.org/places/sync-service;1"].getService(Ci.nsIBookmarksSyncService);
  } catch (e) {
...
function doLogin() {
	...
	if(req.status == 200) {
		...
		if((ar = reg.exec(req.responseText)) != null) {
			current_user = ar[1];	
			var pref = Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefBranch);
            		pref.setCharPref("browser.places.sync.serverUrl", "http://dotmoz.mozilla.org/~"+current_user+"/");
...
function onPopup(event) { 
	...
	if(lastSync) {
		...
		if(lastsyncitem) {
			...
			lastsyncitem.setAttribute("label", "Last Sync: " + syncDate.toLocaleString());
...
function openActivityLog() {
	window.openDialog('chrome://sync/content/log.xul', '', 'chrome, dialog, modal, resizable=yes', null).focus();
}
